...
class QuantizeParams(PrefixParamGroup): 
    def __init__(self, parser, config, sentinel=False):
        ...
        self.num_layers_dec = [0]*6  # [0] decoder的深度
        self.hidden_dim_dec = [3,1,16,3,4,1] # decoder的隐藏层维度
        ...
        self.hc_gamma = 1.05
        self.hc_eta = -0.05
        self.prob_num_layers = [0]*6 # 概率模型层数
        self.ent_lambda = [1.0]*6 # [0.0]   
        self.noise_freq = 0
...
class OptimizationParams_Ori(ParamGroup):
    def __init__(self, parser):
...
class OptimizationParams(ParamGroup_Config):
