...
smirk_id_list = ["xj_new_smirk","ljl_smirk"]
...
for smirk_id in smirk_id_list:
    for quan_bit in quan_bit_list:
        for exp_dim in exp_dim_list:
            command = f'CUDA_VISIBLE_DEVICES="2" /home/xylem/IBC24/CPEM/CPEM-env/bin/python train_smirk_quan.py --idname {smirk_id} --exp_dim {exp_dim} --logname log_quan_{quan_bit}_exp_dim_{str(exp_dim)} --quan_bit {quan_bit}'
