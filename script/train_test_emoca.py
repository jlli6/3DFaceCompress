...
# emoca train
id_emoca_list = ["ljl_wo_glass_emoca"]
id_emoca_list = ["id4_emoca","id5_emoca","id7_emoca","id8_emoca","Obama_emoca","ljl_wo_glass_emoca"]
flame_type = "mica"
for id_name in id_emoca_list:
    command = f'CUDA_VISIBLE_DEVICES="1" /home/xylem/IBC24/CPEM/CPEM-env/bin/python train_emoca.py --idname {id_name} --logname log_mica --flame {flame_type}'
...
for idname in id_emoca_list:
    ...
    for chkpnt in range(55000, 150001, 5000):
        ...
        # 运行 test_smirk.py
        command = f'CUDA_VISIBLE_DEVICES="1" /home/xylem/IBC24/CPEM/CPEM-env/bin/python {script_name} --idname {idname} --checkpoint {checkpoint_file} --logname {logname} --flame {flame_type}'
        process = subprocess.run(command, shell=True)
        if process.returncode != 0:
