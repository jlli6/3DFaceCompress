...
# smirk train
id_smirk_list = ["ljl_wo_glass_smirk"]
flame_type = "mica"
for id_name in id_smirk_list:
    command = f'CUDA_VISIBLE_DEVICES="2" /home/xylem/IBC24/CPEM/CPEM-env/bin/python train_smirk.py --idname {id_name} --logname log_smirk'
...
for idname in id_smirk_list:
    ...
    for chkpnt in range(55000, 150001, 5000):
        ...
        # 运行 test_smirk.py
        command = f'CUDA_VISIBLE_DEVICES="2" /home/xylem/IBC24/CPEM/CPEM-env/bin/python {script_name} --idname {idname} --checkpoint {checkpoint_file} --logname {logname} '
        process = subprocess.run(command, shell=True)
        if process.returncode != 0:
