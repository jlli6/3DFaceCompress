...
class Deform_Model(nn.Module):
    ...
    def init_networks(self):       
        ## full mica 
        self.deformNet = MLP(
            input_dim=self.pts_embedder.dim_embeded + 106, # 120, 184,56, 58
            output_dim=10,
            hidden_dim=256,
            hidden_layers=6
        )
    ...
    def decode(self, codedict):
        ...
        condition = torch.cat((expr_code, jaw_pose), dim=1)
...
class Deform_Model_smirk(nn.Module):
    def __init__(self, device, exp_uesd_dim = 50, exp_dim_used_in50 = 50, mlp_mask = False, multi_mlp=False, mlp_layer = 6, hidden_dim = 256):
        super().__init__()
        self.device = device
    ...
    def smirk_decode(self, codedict,iteration = 0):
        shape_code = codedict['shape'].detach()
        expr_code = codedict['expr'].detach()
        jaw_pose = codedict['jaw_pose'].detach()
        eyelids = codedict['eyelids'].detach()
        ...
        if self.exp_dim_used_in50 != 50:
            expr_code[0,self.exp_dim_used_in50:] = 0
        
        # n = torch.randint(0, 5, (1,)).item()
        # if self.mlp_mask:
        ...
        # if self.multi_mlp:
        #     mlp_id = int(self.exp_dim_used_in50/10) -1
        ...
        #     if self.mlp_mask: # 训练
        #         deforms = self.deformNet[4-n](uv_vertices_shape_embeded_condition)
        #     else: #推理
        #         deforms = self.deformNet[mlp_id](uv_vertices_shape_embeded_condition)
        ...
        # # 带表情的 uv图 
        # uv_vertices_embeded_exp = self.pts_embedder(uv_vertices)
        #  # MLP
        # condition = condition.unsqueeze(1).repeat(1, self.v_num, 1)
       
        # uv_vertices_shape_embeded_condition = torch.cat((uv_vertices_embeded_exp, condition), dim=2)
        # deforms = self.deformNet(uv_vertices_shape_embeded_condition)
        # # print("输入的维度：",uv_vertices_shape_embeded_condition.shape)
        # deforms = torch.tanh(deforms)
        # # print("输出的维度：",deforms.shape)
    ...
    def capture_light(self): 
        # 获取原始模型参数
        state_dict = self.deformNet.state_dict()
        # 将所有参数转换为 fp16
        state_dict_fp16 = {k: v.half() for k, v in state_dict.items()}
        # 返回转换后的参数字典
        return state_dict_fp16
    

    def restore_light(self, model_args): # 只恢复fp16的参数
        net_dict = model_args
        state_dict_fp32 = {k: v.float() for k, v in model_args.items()}
        self.deformNet.load_state_dict(state_dict_fp32)
        # self.deformNet.load_state_dict(net_dict)
        # self.optimizer.load_state_dict(opt_dict)
    ...
    # def update_optimizer_after_pruning(self):
    #     # 冻结剪枝后的权重
    #     for name, param in self.deformNet.named_parameters():
    #         if param.requires_grad == False:  # 已冻结的权重
    #             param.requires_grad = False
        
    #     # 重新定义优化器，只优化仍然requires_grad的权重
    #     self.optimizer = torch.optim.Adam(
    #         filter(lambda p: p.requires_grad, self.deformNet.parameters()),  # 只包含requires_grad=True的权重
    #         lr=1e-4,
    #         betas=(0.9, 0.999)
    #     )
    ...
    def get_pruned_params(self):
        return self._pruned_params if hasattr(self, '_pruned_params') else []

    def update_optimizer_after_pruning(self):
        # 获取所有被剪掉的参数
        pruned_params = self.get_pruned_params()  # 假设这是一个返回被剪枝掉的权重列表的方法

        # 遍历优化器的参数组，找到剪枝掉的参数并将其梯度设置为 0
        for param_group in self.optimizer.param_groups:
            for param in param_group['params']:
                if param in pruned_params:
                    param.grad = None  # 确保这些权重不会参与更新
        print("pruned_params:",pruned_params)
...
class Deform_Model_emoca(nn.Module):
    ...
    def emoca_decode(self, codedict, iteration = 0):
        ...
        # if self.mlp_mask:
        #     probs = torch.tensor([0.4, 0.1, 0.1, 0.2, 0.2]) 
...
class MLP(nn.Module):
    ...
    def prune_mlp(self, pruning_ratio=0.8):
        """
        对模型的权重进行剪枝
        :param pruning_ratio: 剪枝比例 (0 到 1)
        """
        with torch.no_grad():
            for layer in self.fcs + [self.output_linear]:
                # 获取权重绝对值排序
    ...
    def prune_and_freeze(self, pruning_ratio=0.2):
        # 对模型进行剪枝
        for layer in self.fcs:
            prune.l1_unstructured(layer, name="weight", amount=pruning_ratio)
        prune.l1_unstructured(self.output_linear, name="weight", amount=pruning_ratio)

        pruned_params = [] # 保存被剪枝掉的权重
        
        # 确保权重被掩码影响，并冻结剪掉的权重
        for layer in self.fcs + [self.output_linear]:
            mask = dict(layer.named_buffers()).get('weight_mask', None)
            if mask is not None:
                weight_orig = dict(layer.named_parameters()).get('weight_orig', None)
                if weight_orig is not None:
                    # 将剪枝后的权重应用掩码并冻结剪枝掉的部分
                    weight_orig.data *= mask  # 确保掩码生效
                    weight_orig.requires_grad = False  # 冻结被剪枝的权重
                    
                    pruned_params.append(weight_orig[mask == 0])
                    
                    
        # 清除剪枝后的冗余部分（去掉掩码，保持原模型结构）
        for layer in self.fcs:
            prune.remove(layer, 'weight')
        prune.remove(self.output_linear, 'weight')

        # 再次冻结被剪掉的权重
        for name, param in self.named_parameters():
            if 'weight' in name and torch.all(param.abs() < 1e-6):  # 确保为接近 0 的权重
                param.requires_grad = False  # 冻结剪去的权重
                
                
        self._pruned_params = pruned_params

        return self
    
    
class MultiMLP(nn.Module):
    def __init__(self, input_dims, output_dim, hidden_dim=256, hidden_layers=8):
        """
        :param input_dims: 一个包含5个输入维度的列表，例如 [10, 20, 30, 40, 50]
        ...
        self.mlps = nn.ModuleDict({
            f"mlp_{input_dim}": MLP(input_dim, output_dim, hidden_dim, hidden_layers)
    ...
    def forward(self, input):
        """
        :param input: 输入张量，形状为 B, N_v, input_dim
        """
        input_dim = input.shape[-1]
        if input_dim not in self.input_dims:
            raise ValueError(f"Unsupported input_dim: {input_dim}. Supported dimensions: {self.input_dims}")
        mlp = self.mlps[f"mlp_{input_dim}"]  # 根据输入维度选择对应的 MLP
        return mlp(input)
